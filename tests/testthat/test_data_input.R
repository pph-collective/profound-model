...
test_data_input <- function() {
  ...
  races <- dplyr::count(as.data.frame(data$races), data$races)
  ...
  # decision tree
  expect_equal(data$od_wit_priv, 0.492)
  
  # mortality
  expect_equal(data$mor_bl, 0.059, tolerance = 0.001)
  
  # naloxone
  woonsocket <- data.frame(high = c(5, 16, 27, 32, 13))
  woonsocket$low <- c(0, 0, 0, 0, 3)
  rownames(woonsocket) <- 2015:2019
  rownames(woonsocket) <- as.character(rownames(woonsocket))
  x <- as.data.frame(data$nx_oend[, , "Charlestown"])
