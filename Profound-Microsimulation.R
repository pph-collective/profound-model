...
source("Profound-Input.R")

tic()
# INPUT PARAMETERS
set.seed(2021)                          # set the seed 
...
n.pop.t     <- 1059000 * (1 - 0.136)                                               # total population size aged 12 and older (13.6%)
...
MicroSim <- function(init.pop, n.pwud, n.t, v.state, d.c, PT.out = TRUE, Str = "SQ", seed = 1) {
  ...
  for (t in 2:n.t) {
    ...
    pop.list[[t]]$age <- pop.list[[t]]$age + floor(t/12) 
