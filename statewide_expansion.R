...
od_death.mx.last <- od_death.mx.wtns <- matrix(0, nrow = length(simulation_seed), ncol = length(scenario.name))
colnames(od_death.mx.last) <- colnames(od_death.mx.wtns) <- scenario.name
...
write.csv(od_death.mx.last, file = ("Ignore/SA/SA_Expand_3Y_TotalODdeaths.csv"), row.names = F)
write.csv(od_death.mx.wtns, file = ("Ignore/SA/SA_Expand_3Y_WitnessedDeaths.csv"), row.names = F)
