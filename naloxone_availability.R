...
nlx.avail.algm <- function(n.nlx, crosstab_race_resid, OD_loc_pub, nlx.adj, cap, eff.pharNlx, strategy = "SQ", t) {
    n.nlx.total <- n.nlx$OEND + n.nlx$Pharm / eff.pharNlx
    p.nlx.avail <- cap * (1 - exp(-(nlx.adj / cap) * n.nlx.total / crosstab_race_resid))
    return(p.nlx.avail)
}
