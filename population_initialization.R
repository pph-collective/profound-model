...
initiate_ppl <- function(data, agent_states, seed = 2021) {
  ...
  init_ppl <- data.frame(matrix(0, (ppl_size), length(ppl_info) + 1))

  for (i in 1:ppl_size) {
    set.seed(seed + i)
    # TODO read in and clean up
    residence <- sample(p_region$region, 1, prob = p_region$prob)

    # get age/race/sex
    age_probs <- p_region[residence, -c(1, 2)]
    # print(age_probs)
    ...
    oud_prob <- OUDDemo$pe[OUDDemo$age == age & OUDDemo$sex == sex & OUDDemo$race == race]
    stim_prob <- StimDemo$pe[StimDemo$sex == sex & StimDemo$race == race & StimDemo$age == age]
    ...
    if (runif(1) < oud_prob) {
      ...
      curr.state <- init.state <- oud.state[sample(1:length(oud.state), size = 1, prob = oud.prob)]
    } else {
      curr.state <- init.state <- OU.state <- "NODU"
    }
    age <- round(runif(1, substr(age, 1, 2), as.integer(substr(age, nchar(age) - 1, nchar(age)))))
    print(age)
  }
