...
for (ss in 1:length(sim.seed)){
  ...
  sim_sq          <- MicroSim(init.pop, vparameters = vparameters.temp, timesteps, v.state, d.c, PT.out = FALSE, Str = "SQ", seed = sim.seed[ss])        # run for status quo
  ...
  for (ll in 1:dim(pg.add.array)[1]){
    ...
    sim_pg  <- MicroSim(init.pop, vparameters = vparameters.temp, timesteps, v.state, d.c, PT.out = FALSE, Str = "program", seed = sim.seed[ss]) # run for program scenario
